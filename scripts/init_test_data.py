...
class TestDataInitializer:
    ...
    def __init__(self):
        ...
        self.database_url = os.getenv(
            "DATABASE_URL",
            "postgresql+asyncpg://testuser:testpass123@postgres-test:5432/knowledge_rag_test",
        )
    ...
    async def _setup_elasticsearch_indices(self):
        ...
        document_mapping = {
            ...
            "settings": {
                ...
                "analysis": {
                    "analyzer": {
                        "ik_max_word": {"type": "standard"},
                        "ik_smart": {"type": "standard"},
                    }
                },
            },
        }
        ...
        # 创建索引
        indices = [("documents", document_mapping), ("document_chunks", chunk_mapping)]
    ...
    async def _insert_elasticsearch_test_data(self):
        ...
        for doc in documents:
            await self.es_client.index(index="documents", id=doc["id"], body=doc)
        ...
        for chunk in chunks:
            await self.es_client.index(
                index="document_chunks", id=chunk["id"], body=chunk
            )
    ...
    async def _verify_test_data(self):
        ...
        async with self.pg_pool.acquire() as conn:
            ...
            logger.info(
                f"PostgreSQL数据统计: 用户={user_count}, 文档={doc_count}, 消息={msg_count}"
            )
        ...
        # 验证Redis数据
        session_exists = await self.redis_client.exists(
            "session:test_session_token_123"
        )
        cache_exists = await self.redis_client.exists(
            "document:660e8400-e29b-41d4-a716-446655440001"
        )

        logger.info(
            f"Redis数据统计: 会话存在={session_exists}, 缓存存在={cache_exists}"
        )
        ...
        logger.info(
            f"Elasticsearch数据统计: 文档={doc_count['count']}, 块={chunk_count['count']}"
        )
        ...
        async with self.neo4j_driver.session() as session:
            result = await session.run(
                "MATCH (n) WHERE n.test_data = true RETURN labels(n) as labels, count(n) as count"
            )
            neo4j_stats = {}
            async for record in result:
