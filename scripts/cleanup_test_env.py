...
class TestEnvironmentCleaner:
    ...
    async def _cleanup_elasticsearch(self):
        ...
        try:
            ...
            test_indices = [
                ...
                if "test" in idx["index"]
                or idx["index"] in ["documents", "document_chunks"]
            ]
    ...
    async def _cleanup_neo4j(self):
        ...
        try:
            async with self.neo4j_driver.session() as session:
                ...
                # 删除所有测试数据节点和关系
                result = await session.run(
                    "MATCH (n) WHERE n.test_data = true DETACH DELETE n RETURN count(n) as deleted"
                )
                async for record in result:
    ...
    async def _verify_cleanup(self):
        ...
        if self.pg_pool:
            try:
                async with self.pg_pool.acquire() as conn:
                    test_users = await conn.fetchval(
                        "SELECT COUNT(*) FROM users WHERE email LIKE '%@test.com'"
                    )
                    test_docs = await conn.fetchval(
                        "SELECT COUNT(*) FROM documents WHERE title LIKE '%测试%'"
                    )
        ...
        if self.neo4j_driver:
            try:
                async with self.neo4j_driver.session() as session:
                    result = await session.run(
                        "MATCH (n) WHERE n.test_data = true RETURN count(n) as count"
                    )
                    async for record in result:
                        if record["count"] > 0:
                            issues.append(
                                f"Neo4j中仍有 {record['count']} 个测试数据节点"
                            )
            except Exception as e:
