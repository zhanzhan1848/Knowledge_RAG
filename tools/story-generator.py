...
class StoryValidator:
    ...
    def validate_story(self, story_path):
        ...
        # 检查基本结构
        required_sections = ["## Story", "## Acceptance Criteria", "## Tasks / Subtasks", "## Dev Notes"]
        ...
        status = "READY" if len(issues) == 0 else "NEEDS REVISION" if len(issues) <= 2 else "BLOCKED"
...
def main(epic_number=1):
    ...
    if epic_number == 1:
        ...
        generated_files = generator._generate_stories_from_data(generator._get_epic3_data())
        report_name = "epic3-story-validation-report.md"
        epic_name = "Epic 3"
    elif epic_number == 4:
        print("正在生成Epic 4的用户故事...")
        generated_files = generator._generate_stories_from_data(generator._get_epic4_data())
    ...
    for filepath in generated_files:
        ...
        if "error" not in result:
            ...
            print(f"  {status_icon} {Path(filepath).name} - {result['overall_status']} (评分: {result['clarity_score']}/10)")
    ...
    print(f"   成功率: {ready_count/total_count*100:.1f}%" if total_count > 0 else "   成功率: 0%")
