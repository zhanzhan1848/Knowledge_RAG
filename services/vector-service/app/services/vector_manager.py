...
class WeaviateStore(VectorStore):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if not self.client.schema.exists(self.class_name):
                class_definition = weaviate_schema.get_document_class_definition(
                    weaviate_config
                )
            ...
            with self.client.batch as batch:
                ...
                for doc in documents:
                    ...
                    data_object = {
                        ...
                        "document_type": (
                            doc.metadata.get("document_type", "text")
                            if doc.metadata
                            else "text"
                        ),
                        "chunk_index": (
                            doc.metadata.get("chunk_index", 0) if doc.metadata else 0
                        ),
        ...
        try:
            ...
            query_builder = (
                self.client.query.get(
                    self.class_name, ["title", "content", "source", "metadata"]
                )
            ...
            if (
                ...
                and self.class_name in result["data"]["Get"]
            ):
    ...
    async def update_document(self, document: VectorDocument) -> bool:
        ...
        try:
            ...
            data_object = {
                "title": (
                    document.metadata.get("title", "") if document.metadata else ""
                ),
                "content": document.content,
                "source": (
                    document.metadata.get("source", "") if document.metadata else ""
                ),
                "document_type": (
                    document.metadata.get("document_type", "text")
                    if document.metadata
                    else "text"
                ),
                "chunk_index": (
                    document.metadata.get("chunk_index", 0) if document.metadata else 0
                ),
    ...
    async def get_document(self, document_id: str) -> Optional[VectorDocument]:
        ...
        try:
            result = (
                ...
                .with_where(
                    {"path": ["id"], "operator": "Equal", "valueText": document_id}
                )
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                "data" in result
                and "Aggregate" in result["data"]
                and self.class_name in result["data"]["Aggregate"]
            ):
            ...
            return {"backend": "weaviate", "status": "error", "error": str(e)}
...
class VectorManager:
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        try:
            vector_stats = (
                await self.vector_store.get_stats() if self.vector_store else {}
            )
