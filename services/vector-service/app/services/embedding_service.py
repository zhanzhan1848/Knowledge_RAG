...
class OpenAIEmbeddingProvider(EmbeddingProvider):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"OpenAI 嵌入提供者初始化成功，模型: {self.model}，维度: {self.dimensions}"
            )
    ...
    async def embed_text(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.debug(
                f"OpenAI 嵌入完成，处理时间: {processing_time:.3f}s，tokens: {response.usage.total_tokens}"
            )
    ...
    async def embed_texts(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            logger.debug(
                f"OpenAI 批量嵌入完成，处理时间: {processing_time:.3f}s，texts: {len(texts)}，tokens: {response.usage.total_tokens}"
            )
...
class HuggingFaceEmbeddingProvider(EmbeddingProvider):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"HuggingFace 嵌入提供者初始化成功，模型: {self.model_name}，维度: {self.dimensions}"
            )
    ...
    async def embed_texts(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            logger.debug(
                f"HuggingFace 批量嵌入完成，处理时间: {processing_time:.3f}s，texts: {len(texts)}"
            )
...
class SentenceTransformersEmbeddingProvider(EmbeddingProvider):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Sentence Transformers 嵌入提供者初始化成功，模型: {self.model_name}，维度: {self.dimensions}"
            )
    ...
    async def embed_text(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.debug(
                f"Sentence Transformers 嵌入完成，处理时间: {processing_time:.3f}s"
            )
    ...
    async def embed_texts(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            embeddings = self.model.encode(
                texts, convert_to_numpy=True, batch_size=settings.EMBEDDING_BATCH_SIZE
            )
            ...
            logger.debug(
                f"Sentence Transformers 批量嵌入完成，处理时间: {processing_time:.3f}s，texts: {len(texts)}"
            )
...
class EmbeddingService:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"嵌入服务初始化成功，使用提供者: {settings.EMBEDDING_PROVIDER}"
            )
        ...
        try:
            ...
            if use_cache:
                await self.cache_manager.set(
                    cache_key, vector, ttl=settings.EMBEDDING_CACHE_TTL
                )
    ...
    async def embed_texts(
        ...
        try:
            ...
            if uncached_texts:
                ...
                # 更新结果和缓存
                for i, (idx, vector) in enumerate(
                    zip(uncached_indices, uncached_vectors)
                ):
                    vectors[idx] = vector
                ...
                self.metrics_collector.increment(
                    "embeddings_generated", len(uncached_texts)
                )
                self.metrics_collector.record(
                    "batch_embedding_processing_time", processing_time
                )
