...
class DocumentsInput(BaseModel):
    ...
    documents: List[DocumentInput] = Field(
        ..., min_items=1, max_items=100, description="文档列表"
    )
...
class BatchEmbeddingRequest(BaseModel):
    ...
    texts: List[str] = Field(
        ..., min_items=1, max_items=100, description="待嵌入文本列表"
    )
...
async def vector_search(
    ...
    try:
        ...
        results = await vector_manager.vector_store.search(
            query_vector=request.vector, top_k=request.top_k, filters=request.filters
        )
...
async def delete_documents(
    ...
    try:
        ...
        metrics_collector.record(
            "api_delete_documents_processing_time", processing_time
        )
...
async def generate_embedding(
    ...
    try:
        ...
        return EmbeddingResponse(
            ...
            model=(
                embedding_service.provider.model_name
                if hasattr(embedding_service.provider, "model_name")
                else "unknown"
            ),
            processing_time=processing_time,
            timestamp=datetime.now().isoformat(),
...
async def generate_batch_embeddings(
    ...
    try:
        ...
        metrics_collector.record(
            "api_batch_embeddings_processing_time", processing_time
        )
...
async def health_check(
    ...
    try:
        ...
        overall_status = (
            "healthy"
            if vector_manager_healthy and embedding_service_healthy
            else "unhealthy"
        )
        ...
        return HealthResponse(
            ...
            services={
                ...
                "embedding_service": (
                    "healthy" if embedding_service_healthy else "unhealthy"
                ),
            },
        )
