...
class GraphRAGServiceSettings(BaseSettings):
    ...
    GRAPHRAG_WORK_DIR: str = Field(
        ...
        description="GraphRAG工作目录"
    )
    
    # GraphRAG配置文件
    GRAPHRAG_CONFIG_FILE: Optional[str] = Field(
        default=None,
        description="GraphRAG配置文件路径"
    )
    
    # GraphRAG索引配置
    GRAPHRAG_INDEX_TYPE: str = Field(
        default="file",
        description="GraphRAG索引类型"
    )
    
    GRAPHRAG_INDEX_PATH: str = Field(
        default="./data/graphrag/output",
        description="GraphRAG索引路径"
    )
    
    # GraphRAG查询配置
    GRAPHRAG_QUERY_TYPE: str = Field(
        default="global",
        description="GraphRAG查询类型 (global/local)"
    )
    ...
    GRAPH_MAX_RELATIONSHIPS: int = Field(
        default=50000,
        description="最大关系数量"
    )
    ...
    # 本地模型配置
    LOCAL_LLM_ENDPOINT: Optional[str] = Field(
        default=None,
        description="本地LLM端点"
    )
    ...
    SENTENCE_TRANSFORMERS_MODEL: str = Field(
        ...
        description="Sentence Transformers模型"
    )
    ...
    STORAGE_TYPE: str = Field(
        default="local",
        description="存储类型 (local/s3/azure)"
    )
