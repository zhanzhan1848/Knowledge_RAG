...
class Neo4jManager:
    ...
    async def _execute_write_query(self, query: str, parameters: Dict = None) -> List[Dict]:
        """执行写入查询"""
        if not self.connected or not self.driver:
            raise Exception("Neo4j未连接")
    ...
    async def create_community(self, community_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if result:
                ...
                entity_ids = community_data.get("entity_ids", [])
                if entity_ids:
                    await self._link_community_entities(community_data.get("id"), entity_ids)
    ...
    async def get_communities(self, limit: int = 100, offset: int = 0,
    ...
    async def get_entity_neighbors(self, entity_id: str, depth: int = 1, 
        ...
        try:
            ...
            for key, query in queries.items():
                try:
                    ...
                    if key in ["entities_count", "relationships_count", "communities_count", "avg_degree", "max_degree"]:
                        statistics[key] = result[0][list(result[0].keys())[0]] if result else 0
                    else:
                        statistics[key] = result
                except Exception as e:
                    logger.warning(f"获取统计信息失败 {key}: {e}")
                    statistics[key] = 0 if key.endswith("_count") or key.startswith(("avg_", "max_")) else []
