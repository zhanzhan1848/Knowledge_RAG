...
class GraphRAGManager:
    ...
    def _create_default_config(self) -> GraphRagConfig:
        ...
        config_dict = {
            ...
            "cache": {
                "type": "file",
                "base_dir": str(self.work_dir / "cache")
            },
            "reporting": {
                "type": "file",
                "base_dir": str(self.work_dir / "reports")
            },
            "entity_extraction": {
    ...
    async def build_index(self, input_files: List[str], force_rebuild: bool = False) -> Dict[str, Any]:
        """构建GraphRAG索引"""
        try:
        ...
        try:
            ...
            result = await asyncio.get_event_loop().run_in_executor(
                ...
                lambda: self.global_search.search(
                    query=query,
                    community_level=kwargs.get('community_level', settings.GRAPHRAG_COMMUNITY_LEVEL),
                    response_type=kwargs.get('response_type', settings.GRAPHRAG_RESPONSE_TYPE),
                    **kwargs
                )
            )
    ...
    async def local_search_query(self, query: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.local_search.search(
                    query=query,
                    **kwargs
                )
            )
    ...
    async def get_index_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.index_path.exists():
                ...
                # 检查索引文件
                index_files = {
                    "entities": (self.index_path / "create_final_entities.parquet").exists(),
                    "relationships": (self.index_path / "create_final_relationships.parquet").exists(),
                    "communities": (self.index_path / "create_final_communities.parquet").exists(),
                    "community_reports": (self.index_path / "create_final_community_reports.parquet").exists(),
                    "text_units": (self.index_path / "create_final_text_units.parquet").exists()
                }
                ...
                if all(index_files.values()):
                    try:
                        ...
                        entities_df = pd.read_parquet(self.index_path / "create_final_entities.parquet")
                        relationships_df = pd.read_parquet(self.index_path / "create_final_relationships.parquet")
                        communities_df = pd.read_parquet(self.index_path / "create_final_communities.parquet")
    ...
    async def get_relationships(self, limit: int = 100, offset: int = 0) -> Dict[str, Any]:
        """获取关系列表"""
        try:
            if not self.index_path.exists():
                return {
    ...
    async def get_communities(self, limit: int = 100, offset: int = 0) -> Dict[str, Any]:
        """获取社区列表"""
        try:
            if not self.index_path.exists():
                return {
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            status = {
                ...
                "index_exists": self.index_path.exists() and any(self.index_path.iterdir()),
            ...
            # 检查整体健康状态
            status["healthy"] = (
                status["graphrag_available"] and
                status["work_dir_writable"] and
                status["initialized"]
            )
