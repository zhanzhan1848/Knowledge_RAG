...
class EntityInput(BaseModel):
    ...
    community_ids: Optional[List[str]] = Field(
        default_factory=list, description="所属社区ID列表"
    )
...
class RelationshipInput(BaseModel):
    ...
    text_unit_ids: Optional[List[str]] = Field(
        default_factory=list, description="文本单元ID列表"
    )
...
class CommunityInput(BaseModel):
    ...
    entity_ids: Optional[List[str]] = Field(
        default_factory=list, description="实体ID列表"
    )
    relationship_ids: Optional[List[str]] = Field(
        default_factory=list, description="关系ID列表"
    )
...
async def build_graph_index(
    ...
    try:
        ...
        background_tasks.add_task(
            graphrag_manager.build_index, documents=[document.dict()], task_id=task_id
        )
...
async def batch_build_graph_index(
    ...
    try:
        ...
        background_tasks.add_task(
            graphrag_manager.build_index, documents=documents, task_id=task_id
        )
...
async def graphrag_query(
    ...
    try:
        ...
        if result.get("success"):
            ...
            raise HTTPException(
                status_code=400, detail=result.get("message", "查询失败")
            )
...
async def create_entity(
    entity: EntityInput, neo4j_manager: Neo4jManager = Depends(get_neo4j_manager)
):
    """创建新实体"""
    try:
        ...
        if result.get("success"):
            ...
            raise HTTPException(
                status_code=400, detail=result.get("message", "创建实体失败")
            )
...
async def create_relationship(
    ...
    try:
        ...
        if result.get("success"):
            ...
            raise HTTPException(
                status_code=400, detail=result.get("message", "创建关系失败")
            )
...
async def get_relationships(
    ...
    try:
        result = await neo4j_manager.get_relationships(
            limit=limit, offset=offset, source_id=source_id, target_id=target_id
        )
...
async def create_community(
    community: CommunityInput, neo4j_manager: Neo4jManager = Depends(get_neo4j_manager)
):
    """创建新社区"""
    try:
        ...
        if result.get("success"):
            ...
            raise HTTPException(
                status_code=400, detail=result.get("message", "创建社区失败")
            )
...
async def search_entities(
    search_query: SearchQuery, neo4j_manager: Neo4jManager = Depends(get_neo4j_manager)
):
    """搜索实体"""
    try:
...
async def get_shortest_path(
    path_query: PathQuery, neo4j_manager: Neo4jManager = Depends(get_neo4j_manager)
):
    """获取两个实体之间的最短路径"""
    try:
...
async def clear_graph(neo4j_manager: Neo4jManager = Depends(get_neo4j_manager)):
    """清空所有图谱数据"""
    try:
        ...
        if result.get("success"):
            ...
            raise HTTPException(
                status_code=500, detail=result.get("message", "清空图谱数据失败")
            )
...
async def health_check(
    ...
    try:
        ...
        # 综合健康状态
        overall_healthy = graphrag_health.get("healthy", False) and neo4j_health.get(
            "healthy", False
        )
        ...
        return JSONResponse(
            ...
            content={
                ...
                "services": {"graphrag": graphrag_health, "neo4j": neo4j_health},
                "timestamp": datetime.now().isoformat(),
            },
        )
...
async def get_metrics(settings: Settings = Depends(get_config_settings)):
    """获取服务性能指标"""
    try:
        # 获取指标数据
