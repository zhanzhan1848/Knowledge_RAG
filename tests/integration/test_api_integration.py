...
class TestEndToEndWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_document_processing_workflow(self, async_client, sample_document):
        """测试完整的文档处理工作流"""
        # 1. 用户登录
        ...
        # 2. 上传文档
        upload_response = await self._mock_document_upload(async_client, headers, sample_document)
        document_id = upload_response["document_id"]
        
        # 3. 等待文档处理完成
        processing_response = await self._mock_document_processing(async_client, headers, document_id)
        ...
        # 6. 知识图谱构建
        kg_response = await self._mock_knowledge_graph_build(async_client, headers, document_id)
    ...
    async def _mock_document_upload(self, async_client, headers, sample_document):
        ...
        with patch('httpx.AsyncClient.post') as mock_post:
            ...
            response = await async_client.post("/documents/upload", files=files, headers=headers)
            return response.json()
    ...
    async def _mock_document_search(self, async_client, headers):
        ...
        with patch('httpx.AsyncClient.post') as mock_post:
            ...
            response = await async_client.post("/documents/search", json=search_data, headers=headers)
            return response.json()
    ...
    async def _mock_knowledge_graph_build(self, async_client, headers, document_id):
        ...
        with patch('httpx.AsyncClient.post') as mock_post:
            ...
            response = await async_client.post("/knowledge-graph/build", json=kg_data, headers=headers)
            return response.json()
