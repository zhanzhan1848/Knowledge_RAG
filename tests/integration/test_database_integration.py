...
class TestPostgreSQLIntegration:
    ...
    async def test_document_crud_operations(self, db_connection, sample_document):
        ...
        updated_doc = await db_connection.fetchrow(
            update_query, "completed", updated_data["processed_at"], document_data["id"]
        )
    ...
    async def test_transaction_operations(self, db_connection):
        ...
        db_connection.transaction.return_value.__aenter__ = AsyncMock(
...
class TestRedisIntegration:
    ...
    async def test_session_management(self, redis_client):
        ...
        await redis_client.set(
            f"session:{session_id}", json.dumps(session_data), ex=3600  # 1小时过期
        )

        # 验证会话设置
        redis_client.set.assert_called_once_with(
            f"session:{session_id}", json.dumps(session_data), ex=3600
        )
    ...
    async def test_caching_operations(self, redis_client):
        ...
        await redis_client.set(
            f"document:{document_id}", json.dumps(document_data), ex=1800  # 30分钟过期
        )
    ...
    async def test_distributed_locks(self, redis_client):
        ...
        lock_acquired = await redis_client.set(
            lock_key, lock_value, nx=True, ex=300  # 只在键不存在时设置  # 5分钟过期
        )
...
class TestDatabaseConsistency:
    ...
    async def test_cache_database_consistency(self, db_connection, redis_client):
        ...
        await redis_client.set(cache_key, json.dumps(db_result, default=str), ex=1800)
        ...
        updated_db_result = await db_connection.fetchrow(
            update_query, "reprocessing", updated_db_document["updated_at"], document_id
        )

        # 同步更新缓存
        await redis_client.set(
            cache_key, json.dumps(updated_db_result, default=str), ex=1800
        )
