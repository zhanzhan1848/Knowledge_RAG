...
class TestLoadTesting:
    ...
    async def test_document_upload_load(self, load_test_client, performance_metrics):
        ...
        print(f"开始负载测试: {concurrent_users} 并发用户，每用户上传 {documents_per_user} 个文档")
        ...
        async def upload_documents_for_user(user_id: int):
            ...
            for doc_index in range(documents_per_user):
                try:
                    ...
                    # 模拟文档内容
                    document_content = f"负载测试文档内容 - 用户{user_id} - 文档{doc_index}" * 100
                    ...
                    with patch('httpx.AsyncClient.post') as mock_post:
                        ...
                        files = {"file": (f"load_test_{user_id}_{doc_index}.txt", document_content, "text/plain")}
        ...
        # 创建并发任务
        user_tasks = [
            upload_documents_for_user(user_id) 
    ...
    async def test_qa_service_load(self, load_test_client, performance_metrics):
        ...
        print(f"开始问答服务负载测试: {concurrent_users} 并发用户，每用户 {questions_per_user} 个问题")
        ...
        async def ask_questions_for_user(user_id: int):
            ...
            for q_index in range(questions_per_user):
                try:
                    ...
                    user_metrics["errors"].append({"question": question, "error": str(e)})
                    performance_metrics["error_count"] += 1
        ...
        user_tasks = [
            ask_questions_for_user(user_id) 
            for user_id in range(concurrent_users)
        ]
    ...
    async def test_search_service_load(self, load_test_client, performance_metrics):
        ...
        print(f"开始搜索服务负载测试: {concurrent_users} 并发用户，每用户 {searches_per_user} 次搜索")
        ...
        async def search_for_user(user_id: int):
            ...
            for search_index in range(searches_per_user):
                try:
                    ...
                    # 选择搜索关键词
                    keyword = search_keywords[(user_id * searches_per_user + search_index) % len(search_keywords)]
...
class TestStressTesting:
    ...
    async def test_system_stress_mixed_load(self, load_test_client):
            ...
            for i in range(operations):
                try:
                    ...
                    with patch('httpx.AsyncClient.post') as mock_post:
                        ...
                        files = {"file": (f"stress_test_{user_id}_{i}.txt", "测试内容" * 50, "text/plain")}
                        response = await client.post("/documents/upload", files=files)
            ...
            for i in range(operations):
                try:
                    ...
                    mock_response = {
                        ...
                        "relationships": [{"subject": "A", "predicate": "关联", "object": "B"}]
                    }
                    ...
                    with patch('httpx.AsyncClient.post') as mock_post:
                        ...
                        response = await client.post("/knowledge-graph/extract-entities", json=kg_data)
        ...
        for user_id in range(load_scenarios["document_upload"]["users"]):
            task = document_upload_scenario(user_id, load_scenarios["document_upload"]["operations"])
        ...
        for user_id in range(load_scenarios["search_requests"]["users"]):
            task = search_scenario(user_id, load_scenarios["search_requests"]["operations"])
            ...
            if i < len(results) and not isinstance(results[i], Exception):
                ...
                if scenario_name not in performance_metrics["scenario_metrics"]:
                    performance_metrics["scenario_metrics"][scenario_name] = {
                        "success": 0, "errors": 0, "response_times": []
                    }
                
                performance_metrics["scenario_metrics"][scenario_name]["success"] += result["success"]
                performance_metrics["scenario_metrics"][scenario_name]["errors"] += result["errors"]
                performance_metrics["scenario_metrics"][scenario_name]["response_times"].extend(result["response_times"])
        ...
        performance_metrics["total_requests"] = performance_metrics["successful_requests"] + performance_metrics["failed_requests"]
        
        # 计算整体指标
        success_rate = (performance_metrics["successful_requests"] / performance_metrics["total_requests"]) * 100
        ...
        assert avg_response_time < 8.0, f"压力测试平均响应时间过长: {avg_response_time}秒"
        ...
        for scenario_name, metrics in performance_metrics["scenario_metrics"].items():
            scenario_success_rate = (metrics["success"] / (metrics["success"] + metrics["errors"])) * 100
            scenario_avg_time = statistics.mean(metrics["response_times"]) if metrics["response_times"] else 0
            ...
            while True:
                try:
                    ...
                    if operation_count % 4 == 0:
                        # 文档上传
                        mock_response = {"document_id": f"mem_test_{operation_count}", "status": "success"}
                        with patch('httpx.AsyncClient.post') as mock_post:
                            mock_post.return_value.status_code = 201
                            mock_post.return_value.json.return_value = mock_response
                        ...
                        with patch('httpx.AsyncClient.post') as mock_post:
                            mock_post.return_value.status_code = 200
                            mock_post.return_value.json.return_value = mock_response
                            await client.post("/qa/ask", json={"question": f"测试问题{operation_count}"})
                        ...
                        with patch('httpx.AsyncClient.post') as mock_post:
                            mock_post.return_value.status_code = 200
                            mock_post.return_value.json.return_value = mock_response
                            await client.post("/documents/search", json={"query": f"搜索{operation_count}"})
                        ...
                        with patch('httpx.AsyncClient.post') as mock_post:
                            mock_post.return_value.status_code = 200
                            mock_post.return_value.json.return_value = mock_response
                            await client.post("/knowledge-graph/extract-entities", json={"document_id": f"doc_{operation_count}"})
