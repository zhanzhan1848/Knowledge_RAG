...
class TestDocumentProcessingBenchmarks:
    ...
    async def test_document_upload_benchmark(self, benchmark_client):
        ...
        for size_name, size_bytes in file_sizes:
            ...
            for i in range(operations):
                try:
                    ...
                    with patch('httpx.AsyncClient.post') as mock_post:
                        ...
                        files = {"file": (f"benchmark_{size_name}_{i}.txt", content, "text/plain")}
            ...
            if response_times:
                ...
                p95_time = sorted_times[int(0.95 * len(sorted_times))] if len(sorted_times) > 1 else sorted_times[0]
                p99_time = sorted_times[int(0.99 * len(sorted_times))] if len(sorted_times) > 1 else sorted_times[0]
        ...
        for result in benchmark_results:
            if "small" in result.operation_name:
                assert result.avg_time < 0.5, f"小文件上传平均时间过长: {result.avg_time:.3f}秒"
                assert result.throughput >= 10.0, f"小文件上传吞吐量过低: {result.throughput:.2f}"
            elif "medium" in result.operation_name:
                assert result.avg_time < 1.0, f"中等文件上传平均时间过长: {result.avg_time:.3f}秒"
                assert result.throughput >= 5.0, f"中等文件上传吞吐量过低: {result.throughput:.2f}"
            elif "large" in result.operation_name:
                assert result.avg_time < 2.0, f"大文件上传平均时间过长: {result.avg_time:.3f}秒"
                assert result.throughput >= 2.0, f"大文件上传吞吐量过低: {result.throughput:.2f}"
            
            assert result.success_rate >= 95.0, f"{result.operation_name} 成功率过低: {result.success_rate:.1f}%"
    ...
    async def test_document_processing_benchmark(self, benchmark_client):
        ...
        document_types = [
            ...
            ("markdown", "# 标题\n\n这是**markdown**文档内容。\n\n- 列表项1\n- 列表项2" * 50, "text/markdown"),
            ("json", json.dumps({"data": [f"item_{i}" for i in range(100)], "metadata": {"type": "test"}}), "application/json")
        ]
        ...
        for doc_type, content, mime_type in document_types:
            ...
            for i in range(operations):
                try:
                    ...
                    document_id = f"process_benchmark_{doc_type}_{i}_{uuid.uuid4().hex[:8]}"
                    ...
                    with patch('httpx.AsyncClient.post') as mock_post:
                        ...
                        files = {"file": (f"benchmark_{doc_type}_{i}.txt", content, mime_type)}
                    ...
                    with patch('httpx.AsyncClient.post') as mock_post:
                        ...
                        process_data = {"document_id": document_id, "processing_options": {"extract_entities": True}}
                        process_response = await client.post("/documents/process", json=process_data)
                    ...
                    if (upload_response.status_code == 201 and 
                        process_response.status_code == 200 and 
        ...
        # 性能基准断言
        for result in benchmark_results:
            assert result.avg_time < 5.0, f"{result.operation_name} 平均处理时间过长: {result.avg_time:.3f}秒"
            assert result.success_rate >= 90.0, f"{result.operation_name} 成功率过低: {result.success_rate:.1f}%"
            assert result.throughput >= 1.0, f"{result.operation_name} 吞吐量过低: {result.throughput:.2f}"
...
class TestSearchBenchmarks:
    ...
    async def test_vector_search_benchmark(self, search_client):
        ...
        search_scenarios = [
            ...
            ("complex", "基于Transformer架构的大规模语言模型在自然语言处理任务中的应用与优化策略", 50),
        ]
            ...
            for i in range(operations):
                try:
                    ...
                    mock_response = {
                        ...
                        "search_time": 0.05 + (len(query) / 1000),  # 模拟搜索时间与查询复杂度相关
        ...
        for result in benchmark_results:
            if "simple" in result.operation_name:
                assert result.avg_time < 0.2, f"简单搜索平均时间过长: {result.avg_time:.3f}秒"
                assert result.throughput >= 20.0, f"简单搜索吞吐量过低: {result.throughput:.2f}"
            elif "medium" in result.operation_name:
                assert result.avg_time < 0.5, f"中等搜索平均时间过长: {result.avg_time:.3f}秒"
                assert result.throughput >= 10.0, f"中等搜索吞吐量过低: {result.throughput:.2f}"
            elif "complex" in result.operation_name:
                assert result.avg_time < 1.0, f"复杂搜索平均时间过长: {result.avg_time:.3f}秒"
                assert result.throughput >= 5.0, f"复杂搜索吞吐量过低: {result.throughput:.2f}"
            
            assert result.success_rate >= 98.0, f"{result.operation_name} 成功率过低: {result.success_rate:.1f}%"
    ...
    async def test_hybrid_search_benchmark(self, search_client):
        ...
        for i in range(operations):
            try:
                ...
                combined_results = []
                for j in range(10):
                    text_score = mock_text_results[j % len(mock_text_results)]["text_score"]
                    vector_score = mock_vector_results[j % len(mock_vector_results)]["vector_score"]
    ...
    async def test_qa_response_benchmark(self, qa_client):
            ...
            for i, question in enumerate(questions):
                ...
                for attempt in range(3):
                    try:
                        ...
                        # 模拟问答响应
                        answer_length = len(question) * 2 + (100 if category == "factual" else 200 if category == "analytical" else 400)
                        
                        mock_response = {
                            "question": question,
                            "answer": f"这是对'{question[:20]}...'的详细回答。" + "详细内容..." * (answer_length // 20),
                            ...
                            "processing_time": 1.0 + (len(question) / 100) + (answer_length / 1000),
                ...
                print(f"   问题 {i+1}: {avg_question_time:.3f}秒 ({'✓' if avg_question_time <= expected_time else '✗'})")
        ...
        for result in benchmark_results:
            if "factual" in result.operation_name:
                assert result.avg_time < 2.5, f"事实性问答平均时间过长: {result.avg_time:.3f}秒"
                assert result.p95_time < 4.0, f"事实性问答P95时间过长: {result.p95_time:.3f}秒"
            elif "analytical" in result.operation_name:
                assert result.avg_time < 4.0, f"分析性问答平均时间过长: {result.avg_time:.3f}秒"
                assert result.p95_time < 6.0, f"分析性问答P95时间过长: {result.p95_time:.3f}秒"
            elif "complex" in result.operation_name:
                assert result.avg_time < 7.0, f"复杂问答平均时间过长: {result.avg_time:.3f}秒"
                assert result.p95_time < 10.0, f"复杂问答P95时间过长: {result.p95_time:.3f}秒"
            
            assert result.success_rate >= 80.0, f"{result.operation_name} 成功率过低: {result.success_rate:.1f}%"
        ...
        for i in range(workflows):
            try:
                ...
                with patch('httpx.AsyncClient.post') as mock_post:
                    ...
                    files = {"file": (f"benchmark_{i}.txt", document_content, "text/plain")}
                    upload_response = await client.post("/documents/upload", files=files)
                ...
                with patch('httpx.AsyncClient.post') as mock_post:
                    ...
                    search_response = await client.post("/documents/search", json=search_data)
...
def generate_performance_report(benchmark_results: List[BenchmarkResult]) -> str:
    ...
    total_success = sum(r.total_operations * r.success_rate / 100 for r in benchmark_results)
