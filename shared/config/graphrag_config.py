...
class GraphRAGConfig:
    ...
    # 实体提取配置
    entity_extraction: GraphRAGEntityExtractionConfig = field(default_factory=GraphRAGEntityExtractionConfig)
    
    # 社区检测配置
    community_reports: GraphRAGCommunityConfig = field(default_factory=GraphRAGCommunityConfig)
    
    # 摘要配置
    summarize_descriptions: GraphRAGSummarizationConfig = field(default_factory=GraphRAGSummarizationConfig)
    ...
    def from_env(cls) -> 'GraphRAGConfig':
        ...
        # 存储配置
        config.storage.type = GraphRAGStorageType(os.getenv('GRAPHRAG_STORAGE_TYPE', 'file'))
        config.storage.base_dir = os.getenv('GRAPHRAG_STORAGE_BASE_DIR', config.storage.base_dir)
        ...
        config.llm.api_key = os.getenv('OPENAI_API_KEY') or os.getenv('GRAPHRAG_LLM_API_KEY')
        config.llm.api_base = os.getenv('GRAPHRAG_LLM_API_BASE')
        config.llm.max_tokens = int(os.getenv('GRAPHRAG_MAX_TOKENS', str(config.llm.max_tokens)))
        config.llm.temperature = float(os.getenv('GRAPHRAG_TEMPERATURE', str(config.llm.temperature)))
        
        # 嵌入配置
        config.embeddings.type = GraphRAGEmbeddingType(os.getenv('GRAPHRAG_EMBEDDING_TYPE', 'openai'))
        config.embeddings.model = os.getenv('GRAPHRAG_EMBEDDING_MODEL', config.embeddings.model)
        config.embeddings.api_key = os.getenv('OPENAI_API_KEY') or os.getenv('GRAPHRAG_EMBEDDING_API_KEY')
        config.embeddings.batch_size = int(os.getenv('GRAPHRAG_EMBEDDING_BATCH_SIZE', str(config.embeddings.batch_size)))
        
        # 分块配置
        config.chunking.size = int(os.getenv('GRAPHRAG_CHUNK_SIZE', str(config.chunking.size)))
        config.chunking.overlap = int(os.getenv('GRAPHRAG_CHUNK_OVERLAP', str(config.chunking.overlap)))
        ...
        # 缓存配置
        config.cache_base_dir = os.getenv('GRAPHRAG_CACHE_BASE_DIR', config.cache_base_dir)
        
        # 输入配置
        config.input_base_dir = os.getenv('GRAPHRAG_INPUT_BASE_DIR', config.input_base_dir)
