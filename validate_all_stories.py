...
class StoryDraftValidator:
    ...
    def validate_story(self, story_path: Path) -> Dict[str, Any]:
        ...
        validation_result["validation_details"]["reference_effectiveness"] = reference_effectiveness
    ...
    def _check_goal_context_clarity(self, content: str) -> Dict[str, Any]:
        ...
        # 检查故事格式
        if not all(phrase in content for phrase in ["**As a**", "**I want**", "**so that**"]):
        ...
        # 检查依赖关系
        if "依赖" not in content and "depends" not in content.lower() and "requires" not in content.lower():
            # 对于非基础故事，应该有依赖说明
    ...
    def _check_technical_guidance(self, content: str) -> Dict[str, Any]:
        ...
        # 检查技术栈说明
        tech_keywords = ["Python", "FastAPI", "Docker", "PostgreSQL", "Redis", "Neo4j", "Weaviate"]
        if not any(keyword in content for keyword in tech_keywords):
            issues.append("缺少具体的技术栈说明")
            score -= 2
        
        # 检查文件/组件说明
        if "文件" not in content and "组件" not in content and "service" not in content.lower():
        ...
        # 检查配置说明
        if "配置" not in content and "config" not in content.lower() and "环境变量" not in content:
    ...
    def _check_self_containment(self, content: str) -> Dict[str, Any]:
        ...
        # 检查核心信息完整性
        required_sections = ["## Story", "## Acceptance Criteria", "## Tasks", "## Dev Notes"]
    ...
    def _check_testing_guidance(self, content: str) -> Dict[str, Any]:
        ...
        # 检查测试类型说明
        test_types = ["单元测试", "集成测试", "端到端测试", "unit", "integration", "e2e"]
    ...
    def _calculate_overall_assessment(self, validation_result: Dict[str, Any]):
        ...
        partial_count = sum(1 for detail in details.values() if detail["status"] == "PARTIAL")
    ...
    def generate_comprehensive_report(self, validation_results: List[Dict[str, Any]]) -> str:
        """生成综合验证报告"""
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        
        # 统计数据
        total_stories = len([r for r in validation_results if "error" not in r])
        ready_count = sum(1 for r in validation_results if r.get("overall_status") == "READY")
        needs_revision_count = sum(1 for r in validation_results if r.get("overall_status") == "NEEDS REVISION")
        blocked_count = sum(1 for r in validation_results if r.get("overall_status") == "BLOCKED")
        
        avg_score = sum(r.get("clarity_score", 0) for r in validation_results if "error" not in r) / max(total_stories, 1)
...
"""
    ...
    def _generate_improvement_recommendations(self, validation_results: List[Dict[str, Any]]) -> str:
        """生成改进建议"""
        # 统计常见问题
        issue_counts = {}
        for result in validation_results:
...
def main():
    ...
    needs_revision_count = sum(1 for r in results if r.get("overall_status") == "NEEDS REVISION")
    ...
    print(f"   需要修订: {needs_revision_count} ({needs_revision_count/max(total_stories,1)*100:.1f}%)")
    print(f"   阻塞状态: {blocked_count} ({blocked_count/max(total_stories,1)*100:.1f}%)")
